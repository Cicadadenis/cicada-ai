    ...
    while true; do
        ...
        if command -v ss >/dev/null 2>&1; then
            while IFS= read -r line; do
                ...
                idx=$((idx+1))
                port_lines="${port_lines}$(printf '  %2d) port %-6s  pid %-7s  %s\n' "$idx" "$port" "$pid" "$pname")"
                pids_arr="${pids_arr} $pid"
                ports_arr="${ports_arr} $port"
                names_arr="${names_arr} $pname"
            done < <(ss -tulpn 2>/dev/null | grep -v '^Netid' | grep -v '0.0.0.0:*' | grep 'LISTEN')
        elif command -v lsof >/dev/null 2>&1; then
            while IFS= read -r line; do
                local port pid pname
                port=$(echo "$line" | awk '{print $9}' | grep -oP ':\K[0-9]+$')
        ...
        case "$action" in
            s|S)
                ...
                case "$confirm" in
                    [Yy]*)
                        if kill -9 "$sel_pid" 2>/dev/null; then
            ...
            k|K)
                ...
                if echo "$manual_pid" | grep -qE '^[0-9]+$'; then
                    if kill -9 "$manual_pid" 2>/dev/null; then
                        ...
                        printf "${RED}Не удалось остановить PID %s${NC}\n" "$manual_pid"
                    fi
                else
...
lifecycle_start() {
    ...
    printf "${GREEN}Web chat: http://%s:%s${NC}\n" "$CHAT_HOST" "$SELECTED_PORT"
...
Usage: ./cicada-ai.sh <command> [options]

Commands:
  install          Run full installation (interactive)
  start            Start web server (с выбором порта)
  stop             Stop all AI-CICADA services
  restart          Restart services
  status           Show service status and health
  ports            Менеджер портов — просмотр и остановка процессов
  logs             View service logs (last 50 lines)
  logs-analyze     Анализ логов — выявление проблем и советы по устранению
...
Examples:
  ./cicada-ai.sh install              # First time setup (native)
  ./cicada-ai.sh start                # Start (выбор порта в меню)
  ./cicada-ai.sh ports                # Просмотр занятых портов
  ./cicada-ai.sh logs-analyze         # Анализ ошибок в логах
  ./cicada-ai.sh docker               # Setup Docker files
  ./cicada-ai.sh docker-start         # Start with Docker
...
cicada_do_install() {
    ...
    printf "${GREEN}Run 'cicada start' or './cicada-ao.sh start' to begin${NC}\n"
}
...
cicada_doctor() {
    ...
    if [ $issues -eq 0 ]; then
        ...
        printf "${YELLOW}Run './cicada-ai.sh install' to fix${NC}\n"
    fi
}
...
PKGEOF
    ...
    if [ "$ENV_TYPE" = "termux" ]; then
        ...
        printf "${YELLOW}Termux: installing JS deps without native compilation...${NC}\n"
        export CC=clang
        export CXX=clang++
        ...
        printf "${YELLOW}Trying to compile better-sqlite3 (may fail on Android — that is OK)...${NC}\n"
        if (cd "$CHAT_DIR/node_modules/better-sqlite3" && \
            node-gyp rebuild --release >> "$LOG_FILE" 2>&1); then
            printf "${GREEN}better-sqlite3 compiled successfully${NC}\n"
            DB_FALLBACK=0
        else
            printf "${YELLOW}better-sqlite3 compilation failed — will use in-memory SQLite fallback${NC}\n"
            printf "${YELLOW}Chat history will NOT persist between restarts${NC}\n"
            DB_FALLBACK=1
        fi
    else
...
web() {
    ...
    if command -v lsof >/dev/null 2>&1 && lsof -Pi :\$_WEB_PORT -sTCP:LISTEN -t >/dev/null 2>&1; then
        printf "Port \$_WEB_PORT busy, killing old process...\n"
    ...
    printf "Web chat: http://\${CHAT_IP}:\${_WEB_PORT}\n"
...
aicada() {
    case "\$1" in
        start)
            ...
            CHAT_IP=\$(ip route get 1 2>/dev/null | awk '{for(i=1;i<=NF;i++)if(\$i=="src"){print \$(i+1);exit}}' || hostname -I 2>/dev/null | awk '{print \$1}' || echo "localhost")
            printf "Web: http://\${CHAT_IP}:\${_START_PORT}\n"
