...
resolve_configured_web_port() {
    local port
    port=$(get_state "web_port" 2>/dev/null)
    if [ -z "$port" ] && [ -f "$CHAT_DIR/server.js" ]; then
        port=$(grep -oP 'process\.env\.PORT\)?\s*\|\|\s*\K[0-9]+' "$CHAT_DIR/server.js" 2>/dev/null | head -1)
    fi
...
apply_web_port_to_server_js() {
    ...
    # process.env.PORT || 3000   /   parseInt(process.env.PORT) || 3000
    sed -E -i "s/(process\.env\.PORT\)?[[:space:]]*\|\|[[:space:]]*)[0-9]+/\1${new_port}/g" "$server_js"
    # const PORT = 3000
    sed -E -i "s/(const[[:space:]]+[Pp][Oo][Rr][Tt][[:space:]]*=[[:space:]]*)[0-9]+/\1${new_port}/g" "$server_js"
    # listen(3000, ...) / listen(3000)
    sed -E -i "s/\.listen\([[:space:]]*[0-9]+[[:space:]]*,/.listen(${new_port},/g" "$server_js"
    sed -E -i "s/\.listen\([[:space:]]*[0-9]+[[:space:]]*\)/.listen(${new_port})/g" "$server_js"
...
select_port() {
    ...
    set_state "web_port" "$chosen_port"
...
port_manager_menu() {
    while true; do
        ...
        draw_box \
            ...
            "i) Показать текущий порт web" \
            "d) Сбросить порт web на 3000" \
            "r) Обновить список" \
            "q) Назад"
        printf "\n${YELLOW}Действие: ${NC}"
        ...
        case "$action" in
            ...
            c|C)
                ...
                apply_web_port_to_server_js "$new_port" || {
                    printf "${RED}Не удалось обновить server.js${NC}\n"
                    sleep 2
                    continue
                }
            ...
            i|I)
                local curp
                curp="$(resolve_configured_web_port)"
                printf "${CYAN}Текущий порт web: %s${NC}\n" "$curp"
                sleep 2
                ;;
            d|D)
                local curp
                curp="$(resolve_configured_web_port)"
                if [ "$curp" = "3000" ]; then
                    printf "${YELLOW}Порт уже 3000${NC}\n"
                    sleep 1
                    continue
                fi
                apply_web_port_to_server_js "3000" || {
                    printf "${RED}Не удалось сбросить порт до 3000${NC}\n"
                    sleep 2
                    continue
                }
                printf "${GREEN}Порт сброшен: %s → 3000${NC}\n" "$curp"
                printf "${CYAN}Перезапустите сервер: ./cicada-ai.sh restart${NC}\n"
                sleep 2
                ;;
            r|R) continue ;;
...
lifecycle_start() {
    ...
    [ -z "${ENV_TYPE:-}" ] && detect_env

    if ! command -v node >/dev/null 2>&1 || ! command -v npm >/dev/null 2>&1; then
        printf "${YELLOW}Node.js/NPM не найдены. Устанавливаю...${NC}\n"
        install_nodejs || return 1
    fi
    if ! command -v ollama >/dev/null 2>&1; then
        printf "${YELLOW}Ollama не найден. Устанавливаю...${NC}\n"
        install_ollama || return 1
    fi
    if [ ! -f "$CHAT_DIR/server.js" ] || [ ! -f "$CHAT_DIR/index.html" ]; then
        printf "${YELLOW}Файлы веб-чата отсутствуют. Создаю...${NC}\n"
        create_web_chat || return 1
    fi
    if [ ! -d "$CHAT_DIR/node_modules/jsonwebtoken" ]; then
        printf "${YELLOW}NPM-зависимости отсутствуют. Устанавливаю...${NC}\n"
        install_npm_deps || return 1
    fi

    # Выбор порта интерактивно
    local SELECTED_PORT
    SELECTED_PORT="$(resolve_configured_web_port)"
    select_port "$SELECTED_PORT" SELECTED_PORT
...
cicada_health() {
    printf "${CYAN}Проверка здоровья AI-CICADA${NC}\n"
    [ -z "${ENV_TYPE:-}" ] && detect_env
    printf "  Платформа      : %s\n" "$ENV_TYPE"
    printf "  Каталог        : %s\n" "$CHAT_DIR"
    printf "  Лог            : %s\n" "$LOG_FILE"
    ...
    local cfg_port
    cfg_port="$(resolve_configured_web_port)"
    printf "  Web порт       : %s\n" "$cfg_port"

    if command -v node >/dev/null 2>&1; then printf "  Node.js        : ${GREEN}%s${NC}\n" "$(node --version 2>/dev/null)"; else printf "  Node.js        : ${RED}не найден${NC}\n"; fi
    if command -v npm >/dev/null 2>&1; then printf "  npm            : ${GREEN}%s${NC}\n" "$(npm --version 2>/dev/null)"; else printf "  npm            : ${RED}не найден${NC}\n"; fi
    if command -v python3 >/dev/null 2>&1 || command -v python >/dev/null 2>&1; then printf "  Python         : ${GREEN}OK${NC}\n"; else printf "  Python         : ${RED}не найден${NC}\n"; fi
    if command -v ollama >/dev/null 2>&1; then printf "  Ollama bin     : ${GREEN}OK${NC}\n"; else printf "  Ollama bin     : ${RED}не найден${NC}\n"; fi
    if [ -f "$CHAT_DIR/server.js" ] && [ -f "$CHAT_DIR/index.html" ]; then printf "  Web файлы      : ${GREEN}OK${NC}\n"; else printf "  Web файлы      : ${RED}отсутствуют${NC}\n"; fi
    if [ -d "$CHAT_DIR/node_modules" ]; then printf "  node_modules   : ${GREEN}есть${NC}\n"; else printf "  node_modules   : ${RED}нет${NC}\n"; fi
    if pgrep -x "ollama" >/dev/null 2>&1; then printf "  Ollama proc    : ${GREEN}запущен${NC}\n"; else printf "  Ollama proc    : ${YELLOW}не запущен${NC}\n"; fi
    if pgrep -f "node.*server.js" >/dev/null 2>&1; then printf "  Web proc       : ${GREEN}запущен${NC}\n"; else printf "  Web proc       : ${YELLOW}не запущен${NC}\n"; fi

    if command -v docker >/dev/null 2>&1; then
        printf "  Docker         : ${GREEN}установлен${NC}\n"
        if command -v docker-compose >/dev/null 2>&1 || docker compose version >/dev/null 2>&1; then
            printf "  Compose        : ${GREEN}доступен${NC}\n"
        else
            printf "  Compose        : ${YELLOW}не найден${NC}\n"
        fi
    else
        printf "  Docker         : ${YELLOW}не установлен${NC}\n"
    fi
    if command -v udocker >/dev/null 2>&1; then printf "  udocker        : ${GREEN}установлен${NC}\n"; else printf "  udocker        : ${YELLOW}не установлен${NC}\n"; fi
    printf "\n"
}
...
cicada_cli() {
    ...
    printf "${CYAN}AI-CICADA Installer v%s${NC}\n" "$SCRIPT_VERSION"
        ...
        help|--help|-h|*)
            ...
            printf "  ${GREEN}%-18s${NC} %s\n" "health"       "Быстрая проверка зависимостей и сервисов"
...
cicada_do_install() {
    ...
    init_state
    ensure_base_tools || {
        printf "${RED}Не удалось подготовить базовые зависимости. Установка остановлена.${NC}\n"
        return 1
    }
    ...
    if [ "$BACKEND" = "llamacpp" ]; then
        ...
        printf "\n"
        draw_box "1) Браузерный чат (web через llama.cpp)" "2) Терминальный режим (llama-cli)" "3) Выход"
        printf "\n${YELLOW}Выбор: ${NC}"
        read -r ch </dev/tty
        case $ch in
            1) launch_llamacpp ;;
            2) launch_llamacpp_terminal ;;
            *) printf "${GREEN}Готово! Для запуска используйте: ./cicada-ai.sh start${NC}\n" ;;
        esac
    else
        select_model
...
_doctor_change_server_port() {
    ...
    apply_web_port_to_server_js "$new_port" || {
        printf "${RED}Не удалось обновить %s${NC}\n" "$server_js"
        return 1
    }
...
docker_compose_cmd() {
    ...
    printf "${RED}Docker Compose не найден (ни docker-compose, ни docker compose).${NC}\n"
    return 1
}
...
if dc ps | grep -q "ai-cicada"; then
    echo "AI-CICADA is already running"
    echo "Run: dc logs -f web"
    exit 0
fi
...
cicada_docker_start() {
    ...
    docker_compose_cmd up --build -d || return 1
...
cicada_udocker_setup() {
    ...
    if [ -z "$PIP_CMD" ]; then
        ...
        if [ -n "$PY_CMD" ]; then
            "$PY_CMD" -m ensurepip --upgrade >> "$LOG_FILE" 2>&1 || true
            PIP_CMD=$(command -v pip3 2>/dev/null || command -v pip 2>/dev/null || echo "")
        fi
    fi
    [ -z "$PIP_CMD" ] && { printf "${RED}pip не найден после установки Python.${NC}\n"; return 1; }

    # 2. Установка udocker
    if ! command -v udocker >/dev/null 2>&1; then
        printf "${YELLOW}Устанавливаю udocker через pip...${NC}\n"
        "$PIP_CMD" install --user udocker >> "$LOG_FILE" 2>&1 || "$PIP_CMD" install udocker >> "$LOG_FILE" 2>&1
...
install_packages_auto() {
    ...
    case $ENV_TYPE in
        termux)
            yes N | pkg install -y "$@" >> "$LOG_FILE" 2>&1
            ;;
        debian|wsl)
            sudo DEBIAN_FRONTEND=noninteractive apt install -y "$@" >> "$LOG_FILE" 2>&1
            ;;
        fedora)
            sudo dnf install -y "$@" >> "$LOG_FILE" 2>&1
            ;;
        arch)
            sudo pacman -S --noconfirm "$@" >> "$LOG_FILE" 2>&1
            ;;
        homeassistant|alpine|wsl-ha)
            apk add --no-cache "$@" >> "$LOG_FILE" 2>&1
            ;;
        opensuse)
            sudo zypper install -y "$@" >> "$LOG_FILE" 2>&1
            ;;
        void)
            sudo xbps-install -y "$@" >> "$LOG_FILE" 2>&1
            ;;
        *)
            return 1
            ;;
...
ensure_python_runtime() {
    ...
    printf "${YELLOW}Python не найден. Устанавливаю автоматически...${NC}\n"
    local py_pkg
    case $ENV_TYPE in
        termux|arch|fedora|void) py_pkg="python" ;;
        debian|wsl|homeassistant|alpine|wsl-ha|opensuse) py_pkg="python3" ;;
        *) py_pkg="python3" ;;
    esac

    if install_packages_auto "$py_pkg" && (command -v python3 >/dev/null 2>&1 || command -v python >/dev/null 2>&1); then
        printf "${GREEN}Python установлен успешно${NC}\n"
        return 0
    fi

    printf "${RED}Не удалось установить Python автоматически. Лог: %s${NC}\n" "$LOG_FILE"
    return 1
}
...
ensure_base_tools() {
    ...
    if ! command -v curl >/dev/null 2>&1 && ! command -v wget >/dev/null 2>&1; then
        printf "${YELLOW}Нет curl/wget. Устанавливаю curl...${NC}\n"
        if install_packages_auto curl; then
            :
        else
            printf "${RED}Не удалось установить curl автоматически${NC}\n"
            missing=1
        fi
    fi
...
press_any_key() {
    center_text "${YELLOW}Нажмите любую клавишу для продолжения...${NC}"
...
draw_box() {
    ...
    [ $padding -lt 0 ] && padding=0
    printf "%${padding}s${MAGENTA}╔" ""
    repeat_char "═" $(( width - 2 ))
    printf "╗${NC}\n"
    for line in "$@"; do
        printf "%${padding}s${MAGENTA}║${NC} ${WHITE}%-56s${NC} ${MAGENTA}║${NC}\n" "" "$line"
    done
    printf "%${padding}s${MAGENTA}╚" ""
    repeat_char "═" $(( width - 2 ))
    printf "╝${NC}\n"
...
show_logo() {
    ...
    printf "${MAGENTA}%s${NC}\n\n" "$line"
    center_text "${BOLD}${CYAN}* AI-CICADA УСТАНОВЩИК v4.0 *${NC}"
    ...
    center_text "${YELLOW}Платформа: ${display_env}${NC}"
...
launch_llamacpp_terminal() {
    printf "${BLUE}Запуск llama-cli в терминальном режиме...${NC}\n"
    printf "${YELLOW}Модель: %s${NC}\n" "$LLAMA_MODEL_FILE"
    printf "${YELLOW}Нажмите Ctrl+C для выхода${NC}\n"
    printf "\n"
    
    # Check if model exists
    if [ ! -f "$LLAMA_MODEL_PATH" ]; then
        printf "${RED}Модель не найдена: %s${NC}\n" "$LLAMA_MODEL_PATH"
        printf "${YELLOW}Сначала скачайте модель через 'install'${NC}\n"
        return 1
    fi
    
    # Run llama-cli in interactive mode
    if command -v llama-cli >/dev/null 2>&1; then
        llama-cli -m "$LLAMA_MODEL_PATH" -c 4096 -n -1 --interactive --color \
            --prompt "Ты AI-CICADA, полезный ассистент. Отвечай на русском языке.\n\nUser: "
    else
        printf "${RED}llama-cli не найден. Установите llama.cpp:${NC}\n"
        printf "${YELLOW}  https://github.com/ggml-org/llama.cpp${NC}\n"
        return 1
    fi
}
...
create_server_js() {
    ...
    "$PY_CMD" - "$CHAT_DIR/server.js" << 'PYEOF'
import sys
...
const ollamaHttp = ollamaUrl.protocol === 'https:' ? https : http;
const ollamaAgent = ollamaUrl.protocol === 'https:'
    ? new https.Agent({ keepAlive: true, maxSockets: 16, keepAliveMsecs: 10000 })
    : new http.Agent({ keepAlive: true, maxSockets: 16, keepAliveMsecs: 10000 });
...
function backupDatabase() {
    try {
        ...
        const dest = path.join(backupDir, 'cicada-' + ts + '.db');
        fs.copyFileSync(DB_PATH, dest);
        const files = fs.readdirSync(backupDir).filter(function(f){ return f.endsWith('.db'); }).sort();
        while (files.length > 10) {
            const old = files.shift();
            try { fs.unlinkSync(path.join(backupDir, old)); } catch(e) {}
        }
        console.log('SQLite backup created: ' + dest);
    } catch(e) {
        console.warn('SQLite backup skipped:', e.message);
    }
}
...
function migrateDB() {
    if (!db) return;
    let ver = 0;
    try { ver = db.pragma('user_version', { simple: true }) || 0; } catch(e) { ver = 0; }
    if (ver < 1) {
        try { db.exec("ALTER TABLE users ADD COLUMN preferences TEXT DEFAULT '{}'"); } catch(e) {}
        try { db.exec("ALTER TABLE users ADD COLUMN total_msgs INTEGER DEFAULT 0"); } catch(e) {}
        try { db.exec("ALTER TABLE chats ADD COLUMN updated_at INTEGER DEFAULT (strftime('%s','now'))"); } catch(e) {}
        try { db.exec("CREATE INDEX IF NOT EXISTS idx_chats_user ON chats(username)"); } catch(e) {}
        try { db.exec("CREATE INDEX IF NOT EXISTS idx_msgs_chat ON messages(chat_id)"); } catch(e) {}
        try { db.pragma('user_version = 1'); } catch(e) {}
        console.log('SQLite migration applied to version 1');
    }
}
...
const server = http.createServer(async function(req, res) {
    ...
    if (req.method === 'GET' && url === '/api/health') {
        return jsonOk(res, { ok: true, service: 'ai-cicada', model: MODEL, ts: Date.now() });
    }
    ...
    if (req.method === 'POST' && url === '/chat') {
        ...
        req.on('end', function() {
            ...
            var options = {
                ...
                port: parseInt(ollamaUrl.port || (ollamaUrl.protocol === 'https:' ? '443' : '80')),
                path: '/api/chat',
                method: 'POST',
                agent: ollamaAgent,
                timeout: 120000,
            ...
            if (typeof res.socket !== 'undefined' && res.socket) res.socket.setNoDelay(true);

            var clientClosed = false;
            res.on('close', function() {
                clientClosed = true;
                if (ollamaReq) {
                    try { ollamaReq.destroy(); } catch(e) {}
                }
            });

            var streamBuffer = '';
            var ollamaReq = ollamaHttp.request(options, function(ollamaRes) {
                ...
                ollamaRes.on('end', function() {
                    ...
                    if (streamBuffer.trim()) {
                        try {
                            var json = JSON.parse(streamBuffer);
                            var text = (json && json.message && json.message.content) || '';
                            if (text) res.write('data: ' + JSON.stringify({ text: text }) + '\n\n');
                            if (json.done) res.write('data: [DONE]\n\n');
                        } catch(e) {}
            ...
            ollamaReq.on('timeout', function() {
                ollamaReq.destroy(new Error('Timeout waiting Ollama response'));
            });
...
create_index_html() {
    ...
    local PY_CMD
    PY_CMD="$(command -v python3 2>/dev/null || command -v python 2>/dev/null)"
    "$PY_CMD" - "$CHAT_DIR/index.html" << 'PYEOF'
import sys
path = sys.argv[1]
...
:root{
  --bg:#05060a;--bg2:#0d1018;--bg3:#151a24;
...
body::before{content:'';position:fixed;inset:0;z-index:0;background-image:linear-gradient(rgba(0,229,255,0.02) 1px,transparent 1px),linear-gradient(90deg,rgba(200,255,0,0.03) 1px,transparent 1px);background-size:42px 42px;pointer-events:none}
body::after{content:'';position:fixed;width:760px;height:760px;border-radius:50%;background:radial-gradient(circle,rgba(0,229,255,0.10) 0%,rgba(200,255,0,0.08) 30%,transparent 72%);top:-240px;right:-210px;pointer-events:none;z-index:0}
...
.card{width:100%;max-width:420px;background:linear-gradient(180deg,rgba(20,28,40,0.95),rgba(12,16,24,0.95));border:1px solid rgba(0,229,255,0.18);border-radius:var(--r2);padding:36px 28px;box-shadow:0 40px 80px rgba(0,0,0,0.6),var(--glow);position:relative;overflow:hidden;backdrop-filter:blur(8px)}
...
.card-logo-icon{width:44px;height:44px;border-radius:12px;background:linear-gradient(135deg,#0d1422,#10253a);display:flex;align-items:center;justify-content:center;flex-shrink:0;border:1px solid rgba(0,229,255,0.25);box-shadow:0 0 22px rgba(0,229,255,0.2)}
...
.sidebar-logo-icon{width:34px;height:34px;border-radius:9px;background:linear-gradient(135deg,#0d1422,#10253a);display:flex;align-items:center;justify-content:center;flex-shrink:0;border:1px solid rgba(0,229,255,0.25)}
...
.welcome-cicada{width:84px;height:84px;border-radius:22px;background:linear-gradient(135deg,#0f1728,#16273f);border:1px solid rgba(0,229,255,0.26);display:flex;align-items:center;justify-content:center;box-shadow:0 0 26px rgba(0,229,255,0.25);animation:float 3s ease-in-out infinite}
...
.cicada-mark{display:inline-flex;align-items:center;justify-content:center}
.cicada-mark svg{display:block}
.cicada-mark .w{stroke:var(--accent2);stroke-width:2;fill:rgba(0,229,255,0.10)}
.cicada-mark .b{fill:var(--accent)}
.cicada-mark .h{fill:var(--accent3)}
...
#stopBtn{display:none;width:36px;height:36px;border-radius:10px;border:1px solid rgba(255,60,172,0.35);background:rgba(255,60,172,0.14);color:var(--accent3);font-size:15px;cursor:pointer;align-items:center;justify-content:center;flex-shrink:0;transition:all .2s}
#stopBtn:hover{background:rgba(255,60,172,0.25)}
...
<div class="page" id="loginPage">
  <div class="card">
    <div class="card-logo">
      <div class="card-logo-icon"><span class="cicada-mark"><svg width="24" height="24" viewBox="0 0 24 24" aria-hidden="true"><ellipse class="w" cx="7.2" cy="9.5" rx="4.2" ry="2.6" transform="rotate(-18 7.2 9.5)"/><ellipse class="w" cx="16.8" cy="9.5" rx="4.2" ry="2.6" transform="rotate(18 16.8 9.5)"/><ellipse class="b" cx="12" cy="13.2" rx="3.4" ry="6.2"/><circle class="h" cx="12" cy="7.1" r="2.1"/></svg></span></div>
  ...
  <div class="app-layout">
    <div class="sidebar" id="sidebar">
      <div class="sidebar-header">
        <div class="sidebar-logo-icon"><span class="cicada-mark"><svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true"><ellipse class="w" cx="7.2" cy="9.5" rx="4.2" ry="2.6" transform="rotate(-18 7.2 9.5)"/><ellipse class="w" cx="16.8" cy="9.5" rx="4.2" ry="2.6" transform="rotate(18 16.8 9.5)"/><ellipse class="b" cx="12" cy="13.2" rx="3.4" ry="6.2"/><circle class="h" cx="12" cy="7.1" r="2.1"/></svg></span></div>
    ...
    <div class="chat-area">
      ...
      <div class="input-area">
        <div class="input-wrap">
          ...
          <button id="stopBtn" onclick="stopGeneration()" title="Остановить генерацию">&#9632;</button>
...
function cicadaSvg(size) {
    var px = size || 20;
    return '<span class="cicada-mark"><svg width="' + px + '" height="' + px + '" viewBox="0 0 24 24" aria-hidden="true"><ellipse class="w" cx="7.2" cy="9.5" rx="4.2" ry="2.6" transform="rotate(-18 7.2 9.5)"/><ellipse class="w" cx="16.8" cy="9.5" rx="4.2" ry="2.6" transform="rotate(18 16.8 9.5)"/><ellipse class="b" cx="12" cy="13.2" rx="3.4" ry="6.2"/><circle class="h" cx="12" cy="7.1" r="2.1"/></svg></span>';
}

var API = {
    _token: function() { return Session.getToken(); },
    _jsonOrThrow: function(r) {
    ...
    _post: function(url, data) {
        return fetch(url, { method:'POST', headers: API._headers(), body: JSON.stringify(data) })
            .then(API._jsonOrThrow);
    },
    _get: function(url) {
        return fetch(url, { headers: { 'Authorization': 'Bearer ' + (API._token() || '') } })
            .then(API._jsonOrThrow);
    },
    _del: function(url) {
        return fetch(url, { method:'DELETE', headers: { 'Authorization': 'Bearer ' + (API._token() || '') } })
            .then(API._jsonOrThrow);
    },
...
function login() {
    ...
    API.login(u, p).then(function(res) {
        if (!res.token) return showError('loginError', 'Ошибка авторизации: токен не получен');
        currentUser = res;
        Session.set(u);
        Session.setToken(res.token);
        enterChat();
    }).catch(function(err) {
        showError('loginError', err.message || 'Ошибка входа');
    });
}
...
function register() {
    ...
    API.register(u, p).then(function(res) {
        API.login(u, p).then(function(r2) {
            if (!r2.token) return showError('regError', 'Ошибка авторизации: токен не получен');
            currentUser = r2; Session.set(u); Session.setToken(r2.token); enterChat();
        }).catch(function(err) {
            showError('regError', err.message || 'Ошибка входа после регистрации');
        });
    }).catch(function(err) {
...
function resetMessages() {
    ...
    m.innerHTML = '<div class="welcome" id="welcomeBlock">' +
        '<div class="welcome-cicada">' + cicadaSvg(52) + '</div>' +
        '<h1>AI-CICADA</h1>' +
...
function send() {
    ...
    function scheduleFlush() {
        ...
        rafScheduled = true;
        requestAnimationFrame(flushAiText);
    }

    generationController = new AbortController();
    fetch('/chat', { method:'POST', headers: API._headers(), body: JSON.stringify({ messages: chatHistory }), signal: generationController.signal })
    .then(function(res) {
        ...
        function read() {
            return reader.read().then(function(x) {
                if (x.done) return;
                sseBuffer += dec.decode(x.value, { stream: true });
    ...
    .catch(function(err) {
        if (err && err.name === 'AbortError') {
            ...
            if (!fullText) addMsg('ai', 'Генерация остановлена.');
            return;
        }
        if (typingEl && typingEl.remove) typingEl.remove();
...
function stopGeneration() {
    if (!generating || !generationController) return;
...
function addMsg(role, text) {
    ...
    av.className = 'avatar';
    if (role === 'user') av.textContent = currentUser ? currentUser.username[0].toUpperCase() : 'Я';
    else av.innerHTML = cicadaSvg(16);
...
function addTyping() {
    ...
    div.innerHTML = '<div class="avatar">' + cicadaSvg(16) + '</div><div class="typing-bubble"><span></span><span></span><span></span></div>';
    m.appendChild(div); m.scrollTop = 999999;
    return div;
}
...
launch_choice() {
    clear
    center_text "${BOLD}${CYAN}Что запустить сейчас?${NC}"
    printf "\n"
    draw_box \
        "1) [WEB] Браузерный чат (web)" \
        "2) [CLI] Терминальный агент (ai)" \
        "3) [X]   Выход"
    printf "\n${YELLOW}Выбор: ${NC}"
    read -r ch </dev/tty
    # Cross-platform IP detection
