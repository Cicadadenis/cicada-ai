...
select_port() {
    ...
    clear
    draw_title "⚙  Выбор порта для веб-сервера" "$CYAN"
    draw_box \
        "1) 3000  (по умолчанию)" \
        "2) 8080" \
        "3) 8888" \
...
port_manager_menu() {
    while true; do
        clear
        draw_title "🔌  Менеджер портов" "$CYAN"
        ...
        if [ -z "$port_lines" ]; then
            ...
            center_text "${YELLOW}▶  Занятые порты:${NC}"
            printf "\n"
            printf "%s" "$port_lines"
        fi
...
press_any_key() {
    printf "\n"
    center_text "${CYAN}[ ${YELLOW}Нажмите любую клавишу для продолжения...${CYAN} ]${NC}"
...
draw_box() {
    ...
    # Top border
    printf "%${padding}s${CYAN}╔" ""
    repeat_char "═" "$inner"
    printf "╗${NC}\n"

    local item_num=0
    for line in "$@"; do
        # Detect if line starts with a digit and ) — colour it
        ...
        if printf '%s' "$line" | grep -qE '^[0-9]+\)'; then
            local num key rest
            num=$(printf '%s' "$line" | grep -oE '^[0-9]+')
            rest=$(printf '%s' "$line" | sed "s/^${num})//" )
            colored_line="${YELLOW}${num}${CYAN})${NC}${rest}"
            # calculate visual length (strip ANSI for padding calc)
            local vis_len=$(( ${#num} + 1 + ${#rest} ))
            local pad_right=$(( inner - 2 - vis_len ))
            [ $pad_right -lt 0 ] && pad_right=0
            printf "%${padding}s${CYAN}║${NC} %b%*s ${CYAN}║${NC}\n" \
                "" "$colored_line" "$pad_right" ""
        elif printf '%s' "$line" | grep -qE '^[sqrpkcaA-Z]\)'; then
            local letter rest2
            letter=$(printf '%s' "$line" | cut -c1)
            rest2=$(printf '%s' "$line" | cut -c2-)
            colored_line="${GREEN}${letter}${CYAN})${NC}${rest2}"
            local vis_len2=$(( 1 + 1 + ${#rest2} ))
            local pad_right2=$(( inner - 2 - vis_len2 ))
            [ $pad_right2 -lt 0 ] && pad_right2=0
            printf "%${padding}s${CYAN}║${NC} %b%*s ${CYAN}║${NC}\n" \
                "" "$colored_line" "$pad_right2" ""
        elif [ -z "$line" ]; then
            printf "%${padding}s${CYAN}║%*s║${NC}\n" "" "$inner" ""
        else
            local vis_len3=${#line}
            local pad_right3=$(( inner - 2 - vis_len3 ))
            [ $pad_right3 -lt 0 ] && pad_right3=0
            printf "%${padding}s${CYAN}║${NC} %s%*s ${CYAN}║${NC}\n" \
                "" "$line" "$pad_right3" ""
        fi
    done
...
select_model() {
    clear
    draw_title "🤖  Выберите модель" "$CYAN"
    if [ "$ENV_TYPE" = "homeassistant" ] || [ "$ENV_TYPE" = "wsl-ha" ]; then
        draw_box \
...
select_backend() {
    clear
    draw_title "⚡  Выберите AI-бэкенд" "$CYAN"
    draw_box \
...
select_llama_model() {
    clear
    draw_title "🦙  Выберите модель llama.cpp" "$CYAN"
    draw_box \
...
start_ollama_service() {
    ...
    if pgrep -x "ollama" > /dev/null 2>&1; then
        printf "${GREEN}Ollama уже работает${NC}\n"
        return
    fi
    case $ENV_TYPE in
        termux)
...
show_ha_tips() {
    ...
    clear
    draw_title "🏠  Home Assistant — советы" "$GREEN"
    draw_box \
...
final_screen() {
    ...
    draw_title "✅  Установка завершена!" "$GREEN"
    draw_box \
...
launch_choice() {
    clear
    draw_title "🚀  Что запустить сейчас?" "$YELLOW"
