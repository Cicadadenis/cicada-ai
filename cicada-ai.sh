...
lifecycle_stop() {
    ...
    if [ $killed -gt 0 ]; then
        printf "${GREEN}Сервисы остановлены${NC}\n"
    else
...
cicada_doctor() {
    ...
    if [ $issues -eq 0 ]; then
        ...
        printf "${YELLOW}Найдено проблем: %d${NC}\n" "$issues"
        printf "${YELLOW}Запустите: ./cicada-ai.sh install  — для исправления${NC}\n"
    fi
}
...
cicada_udocker_start() {
    ...
    # Запрашиваем модель
    printf "${YELLOW}Введите имя модели для загрузки [qwen2.5-coder:3b]: ${NC}"
    read -r _udm </dev/tty
    ...
    case $ENV_TYPE in
        termux)
            if pkg show ollama >/dev/null 2>&1; then
                ...
                printf "${YELLOW}Ollama нет в репозитории pkg, используется proot-distro...${NC}\n"
                ...
                if ! curl -fsSL https://ollama.com/install.sh -o "$INSTALLER" >> "$LOG_FILE" 2>&1; then
                    printf "${RED}Не удалось скачать установщик Ollama${NC}\n"
                    return 1
                fi
                if ! verify_download "$INSTALLER" 1000; then
                    printf "${RED}Ошибка проверки загрузки установщика Ollama${NC}\n"
                    return 1
                fi
                chmod +x "$INSTALLER"
...
aicada() {
    case "\$1" in
        ...
        status)
            ...
            if command -v lsof >/dev/null 2>&1 && lsof -Pi :3000 -sTCP:LISTEN -t >/dev/null 2>&1; then printf "  Порт 3000: занят\n"; else printf "  Порт 3000: свободен\n"; fi
            ;;
        *)
